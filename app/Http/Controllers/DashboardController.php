...
class DashboardController extends Controller
{
    ...
    {  
		if($request->ajax()){
			...
			//$leads= $leads->whereDate('clients.expired_on','<=',date('Y-m-d', strtotime("+15 days")));//DB::raw('DATE_ADD(curdate(),INTERVAL 15 DAY)')
		 
			/*   $leads = $leads->where(function($query){
							$query->where(function($query){
								$query->where('clients.leads_remaining','<',15);
									  //->where('clients.balance_amt','>','0');					
							})
							->orWhere(function($query){
								$query->where(function($query){
									$query->where('clients.expired_on','<=',date('Y-m-d', strtotime("+15 days")));
 ...
 public function getpendingLeadsDashboard(Request $request)
    {
		...
		if($request->ajax()){			
  //	echo "<pre>";print_r($_GET);echo "<pre>";print_r($request->input('search.city'));
		$leads = DB::table('leads as leads');
		//$leads = $leads->join('keyword as keyword','leads.kw_id','=','keyword.id');
		if($request->input('search.value')!=''){
				$leads = $leads->where(function($query) use($request){
...
//echo $request->input('search.status');die;			
...
// echo "<pre>";print_r($leads);die;
		...
		//echo "<pre>";print_r($returnLeads);die;
			return response()->json($returnLeads);
