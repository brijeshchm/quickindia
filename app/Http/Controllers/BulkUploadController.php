...
class BulkUploadController extends Controller
{
    ...
    public function storeBulkUploadKewyword(Request $request){
		...
		//$results = Excel::selectSheets('Keyword')->load($file)->get();
 ...
 /**
	...
	//	echo $user_id;echo "<pre>";print_r($results);die;
		$arr = [];
		foreach($results as $result){
			//echo $result->name;die;			
		//echo substr(trim($result->mobile),-10);die;
			//$arrayToPass = [];
			/* foreach($result as $key=>$value){
				$arrayToPass[$key] = $value;
			}  */
		...
		//	if(!$validator->fails()){				
				...
				//$followUp->expected_date_time = $result->expected_date_time;
